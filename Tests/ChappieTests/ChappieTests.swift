...
@main struct ChappieTests {
    ...
    func testReminderDrafts() {
        ...
        XCTAssertFalse(Intent.isReminderAddition("リマインドを消して"))
        XCTAssertFalse(Intent.isReminderAddition("リマインドをキャンセルして"))
        let bare = Intent.reminderDraft(from: "明日9時にリマインダーして", now: now, calendar: calendar)
        XCTAssertEqual(bare?.title, "リマインド")
        XCTAssertFalse(Intent.isReminderAddition("今日の予定を教えて"))

        // A day without a month is the next such day; the system detector alone drops it.
        let fixed = calendar.date(from: DateComponents(year: 2026, month: 9, day: 22, hour: 10))!
        let dayOnly = Intent.reminderDraft(from: "23日に段ボール捨てるってリマインダー入れといて", now: fixed, calendar: calendar)
        XCTAssertEqual(dayOnly?.title, "段ボール捨てる")
        XCTAssertEqual(dayOnly.map { calendar.dateComponents([.month, .day, .hour], from: $0.due) }, DateComponents(month: 9, day: 23, hour: 9))
        let dayTime = Intent.reminderDraft(from: "段ボール捨てるのを23日の夜8時半にリマインダーに入れて", now: fixed, calendar: calendar)
        XCTAssertEqual(dayTime?.title, "段ボール捨てる")
        XCTAssertEqual(dayTime.map { calendar.dateComponents([.month, .day, .hour, .minute], from: $0.due) }, DateComponents(month: 9, day: 23, hour: 20, minute: 30))
        let passed = Intent.reminderDraft(from: "5日に家賃ってリマインドして", now: fixed, calendar: calendar)
        XCTAssertEqual(passed.map { calendar.dateComponents([.month, .day], from: $0.due) }, DateComponents(month: 10, day: 5))
        XCTAssertNil(Intent.reminderDraft(from: "32日にリマインドして", now: fixed, calendar: calendar))
        XCTAssertNil(Intent.reminderDraft(from: "明日のリマインドを消して", now: fixed, calendar: calendar))
        XCTAssertTrue(Intent.isReminderEdit("明日のリマインドを消して"))
        XCTAssertTrue(Intent.isReminderEdit("リマインダーの時間を変えて"))
        XCTAssertFalse(Intent.isReminderEdit("明日10時にホテルをキャンセルしてってリマインドして"))
        let cancelHotel = Intent.reminderDraft(from: "明日10時にホテルをキャンセルしてってリマインドして", now: fixed, calendar: calendar)
        XCTAssertEqual(cancelHotel?.title, "ホテルをキャンセルして")
        // Said on the 23rd itself, "23日" is today, not next month.
        let onTheDay = calendar.date(from: DateComponents(year: 2026, month: 9, day: 23, hour: 10))!
        let sameDay = Intent.reminderDraft(from: "23日の夜8時に段ボールってリマインドして", now: onTheDay, calendar: calendar)
        XCTAssertEqual(sameDay.map { calendar.dateComponents([.month, .day, .hour], from: $0.due) }, DateComponents(month: 9, day: 23, hour: 20))
        let dayNi = Intent.reminderDraft(from: "23日に9時に段ボール捨てるってリマインダー入れといて", now: fixed, calendar: calendar)
        XCTAssertEqual(dayNi?.title, "段ボール捨てる")
        XCTAssertEqual(dayNi.map { calendar.component(.hour, from: $0.due) }, 9)
        let timeFirst = Intent.reminderDraft(from: "夜8時に23日は段ボール捨てるってリマインダー", now: fixed, calendar: calendar)
        XCTAssertEqual(timeFirst?.title, "段ボール捨てる")
        XCTAssertEqual(timeFirst.map { calendar.dateComponents([.day, .hour], from: $0.due) }, DateComponents(day: 23, hour: 20))
        let fullWidth = Intent.reminderDraft(from: "２３日に9時に段ボールってリマインドして", now: fixed, calendar: calendar)
        XCTAssertEqual(fullWidth.map { calendar.dateComponents([.day, .hour], from: $0.due) }, DateComponents(day: 23, hour: 9))
        // "3日分" is a quantity, not the 3rd; with no other date it is not a reminder.
        XCTAssertNil(Intent.reminderDraft(from: "3日分の薬を飲むってリマインドして", now: fixed, calendar: calendar))
        let thirtyFirst = Intent.reminderDraft(from: "31日に家賃ってリマインドして", now: fixed, calendar: calendar)
        XCTAssertEqual(thirtyFirst.map { calendar.dateComponents([.month, .day], from: $0.due) }, DateComponents(month: 10, day: 31))
